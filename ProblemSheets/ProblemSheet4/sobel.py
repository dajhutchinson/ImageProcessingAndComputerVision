...
def run():
    ...
    # magnitude
    mag=magnitude(convolved_dx,convolved_dy)
    normalised_mag=normalise(mag)
    cv.imwrite("img/magnitude.png",normalised_mag)

    # direction
    dir=direction(convolved_dx,convolved_dy)
    normalised_dir=normalise(dir,min=-np.pi,max=np.pi)
